...
public class DiscoveryLinkingServiceImpl implements DiscoverLinkingService {
  ...
  public void onDiscoverCall(DiscoverResponse data)
      throws URISyntaxException, JsonProcessingException {
    String receivedAbhaAddress = data.getPatient().getId();
    String receivedPatientReference =
        data.getPatient().getUnverifiedIdentifiers().isEmpty()
            ? null
            : data.getPatient().getUnverifiedIdentifiers().get(0).getValue();
    ...
    try {
      ...
      List<Patients> isMobilePresent =
          patientRepo.findByPatientMobile(
              data.getPatient().getVerifiedIdentifiers().get(0).getValue());
      Patients isPatientIdentifier =
      ...
      if (Objects.nonNull(isAbhaPresent)) {
        ...
        List<CareContextBuilder> careContexts =
            isAbhaPresent.getCareContexts().stream()
        ...
        Optional<Patients> matchingPatient =
            findMatchingPatient(
                isMobilePresent,
        ...
        if (isGenderMatch(isPatientIdentifier, receivedGender)
            && (isYOBInRange(isPatientIdentifier, receivedYob)
                && (isFuzzyNameMatch(isPatientIdentifier, receivedName)))) {
          List<CareContextBuilder> careContexts =
              isPatientIdentifier.getCareContexts().stream()
                  .filter(context -> !context.isLinked())
                  .collect(Collectors.toList());
  ...
  public void onInitCall(InitResponse data) throws URISyntaxException, JsonProcessingException {
    ...
    try {
      ResponseEntity<ObjectNode> responseEntity =
          MakeRequest.post(GatewayApiPaths.ON_INIT, requestEntity);
      log.info(GatewayApiPaths.ON_INIT + " : onInitCall: " + responseEntity.getStatusCode());
    } catch (Exception e) {
      log.info(
          GatewayApiPaths.ON_INIT
              + " : OnInitCall -> Error : "
  ...
  public void onConfirmCall(ConfirmResponse data)
      throws URISyntaxException, JsonProcessingException {
    ...
    List<CareContextBuilder> selectedCareContexts =
        logsTableService.getSelectedCareContexts(linkRefNumber, careContexts);
    try {
  ...
  public void makeBody(
      DiscoverResponse data, Patients patients, List<CareContextBuilder> careContexts)
      throws URISyntaxException, JsonProcessingException {
    ...
    try {
      ...
      log.info(
          GatewayApiPaths.ON_DISCOVER + " : onDiscoverCall: " + responseEntity.getStatusCode());
  ...
  private Optional<Patients> findMatchingPatient(
      List<Patients> isMobilePresent,
      String receivedPatientReference,
